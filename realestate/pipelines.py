...
class RealestatePipeline(object):
    def __init__(self):
        ...
        self.conn = MySQLdb.connect(host='127.0.0.1', user='root', passwd='', db='realstatedb', charset='utf8')
        self.conn.ping(True)
    ...
    def process_item(self, item, spider):
        try:
            query = "SELECT * FROM listings WHERE website='{}' AND title='{}'".format(item['website'], item['title'].replace("'", '´'))
            result = self.cursor.execute(query)
            if result > 0:
